...
//三级联动注意事项:1,用$.each(v.city, function(l, m)}时候,m是v.city的下一级2,每次点击事件要确保联动在他的下面执行,不然下一个点击娶不到数值(因为需要在l下找数值.)3,创建成之后就要用html的思维了,这是不能在停留在js创建的思维上啦❗❗️
$(function (){
		$("#header").load("xjx_top.html");
		$("#footer").load("jx_bottom.html");
	});

$('.area_class>input').click(function() {
	$('.ols').css('display','block')
	//省级的内容先找到
	//for(var i in area_address){
	//var lis=$('<li class="pro_li"></li>')
	//	
	//	$(lis).html((area_address[i].name))
	//	
	//lis.appendTo($('.area_class'))
	$("ol").eq($(this).index()).show();

	//	}
});
$.each(area_address, function(i, v) {
	//	$(".sheng").empty()
	...
	$('<p >' + v.name + '</p>').appendTo($('.sheng'))
	$('.sheng p').eq(i).click(function() {
		$('#area1').val($('.sheng p').eq(i).html())

		//创建成之后就要用html的思维了,这是不能在停留在js创建的思维上啦❗️

		$('#area2').click(function() {
			$(".shi p").remove()
			$.each(v.city, function(l, m) {
				$('<p >' + m.name + '</p>').appendTo($(".shi"))
				$(".sheng").hide()
				$(".shi").show() 
				$(".xian").hide()
				$('.shi p').eq(l).click(function() { //他的联动要保证每一个子元素在这个点击事件下进行❗️
					$('#area2').val($('.shi p').eq(l).html())

					$('#area3').click(function() {
						$('.xian p').remove();
						$.each(m.area, function(s, t) {
							console.log(t)
							$('<p >' + t + '</p>').appendTo($(".xian"))
							$(".sheng").hide()
							$(".shi").hide()
							...
							$('.xian p').eq(s).click(function() {
								$('#area3').val($('.xian p').eq(s).html())
								$('.xian').hide();
							})
						})
